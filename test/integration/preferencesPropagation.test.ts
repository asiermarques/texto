...
// US-015: the Prose editor's preferences as VSCode settings.
//
// toggleFocusMode.test.ts already covers the palette command (which goes
// through ProseEditorProvider.setFocusModeEnabled). What this suite adds is
...
suite('US-015: changing texto.modoFoco directly propagates to the open panels', () => {
  ...
  test('editing the setting (without the command) shows up in the open Chapter, without reloading', async () => {
    ...
    await vscode.workspace.getConfiguration('texto').update('modoFoco', false, vscode.ConfigurationTarget.Global);
