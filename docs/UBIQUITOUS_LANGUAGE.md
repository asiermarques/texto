...
| **Padded source** | A **Table**'s markdown with every column widened to its widest **Cell**, leading and trailing pipes on every **Row**, and the **Delimiter row**'s dashes filling the same widths. What the **Author** sees when a **Table** reveals under the cursor, and what the `.md` carries into a diff. Maintained as they type (PD-009), never on open or on save. | Fuente alineada | Padded source | Formatted table, pretty-printed, normalised (it only ever adds or removes spaces) |
...
| **Diagram** | A **Code block** whose info string is `mermaid`: its **Diagram source** describes a picture, and the **Live preview** composes it as that picture. The one construct of the **Composed subset** that is drawn rather than styled — see **Diagram plate** — and, like a **Table**, revealed whole (requirement 010). | Diagrama | Diagram | Gráfico, esquema, figura, mermaid (that is the **Diagram language**, not the thing) |
| **Diagram source** | The text inside a **Diagram**'s fences — a program in the **Diagram language**, not prose. Never counted by the **Word count**, exactly like any other **Code block**'s contents, and never touched: the picture is drawn from it, never written back into the **Chapter**. | Fuente del diagrama | Diagram source | Código del diagrama, definición |
| **Diagram language** | The notation a **Diagram source** is written in: mermaid, named in the fence's info string. | Lenguaje de diagrama | Diagram language | Sintaxis, DSL |
| **Diagram plate** | How a composed **Diagram** is set: an object placed in the prose rather than prose itself — its own ground, its own palette (the renderer's, not the **Editor theme**'s), centred, and scaled down rather than bled when it is wider than the measure. | Lámina del diagrama | Diagram plate | Caja, tarjeta, imagen (an **Image** is a different term and is never rendered — NOGOAL-002 of 006) |
...
| **Word count** | The number of prose words in the open **Chapter**, and in the current selection — neither markdown syntax nor **Frontmatter** counts. | Palabras | Word count | Caracteres, longitud, contador |
...
| **Frontmatter** | Metadata block fencing the very top of a **Chapter** — title, author, tags — written either as YAML between `---` lines or as TOML between `+++` lines. Not prose: it is folded out of the **Writing surface** by default and never counted by the **Word count**. The toolbar's Frontmatter button unfolds it, and folds it away again. | Frontmatter | Frontmatter | Cabecera, metadatos, preámbulo, YAML; and above all **Scene break**, which is written the same way and is not this |
...
| Term | Definition | UI wording (ES) | UI wording (EN) | Avoid |
| --- | --- | --- | --- | --- |
| **Operation count** | A deterministic count — full markdown parses, **Tree update**s and changed document ranges per keystroke (in prose, and again for one landing inside a **Table**'s **Cell**), **Live preview** instructions, **Focus mode** dim ranges, built-bundle bytes — compared for exact equality against a baseline committed to the repository, on every commit (requirement 007, extended by 008 and 009). | — | — | Work counter (collides with **Work**), benchmark, metric (too generic once **Operation count** is the specific term) |
