...
| **Frontmatter** | Metadata block fencing the very top of a **Chapter** — title, author, tags — written either as YAML between `---` lines or as TOML between `+++` lines. Texto never composes or counts it as prose; it only tells the **Author** it is there. | Frontmatter | Frontmatter | Cabecera, metadatos, preámbulo, YAML; and above all **Scene break**, which is written the same way and is not this |
